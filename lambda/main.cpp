...
std::string encode(const std::string &file, unsigned long length) {
  ...
  // multilying the original length by 2 retrofites a segfault
  auto uncompressed = uncompress(reinterpret_cast<const unsigned char*>(decoded.data()), decoded.size(), length * 2);
